...
mol_metrics.py
==============
Standalone molecular evaluation metrics for generative models.

Designed to be framework-agnostic: accepts plain RDKit molecules and SMILES
strings, with no dependency on DeepChem, PyTorch, or any GAN internals.

Future PyPI entry point:
...
# Optional dependency – FCD is heavy; we degrade gracefully if missing.
try:
    from fcd_torch import FCD as _FCDMetric  # Ajuste da importação
    _FCD_AVAILABLE = True
except ImportError:  # pragma: no cover
    _FCD_AVAILABLE = False

# Silence RDKit noise inside the library.
...
# ---------------------------------------------------------------------------
# Public per-molecule helpers  (useful on their own outside evaluate_molecules)
# ---------------------------------------------------------------------------
...
def evaluate_molecules(
    ...
    *,
    ...
    unique_smiles  = list(dict.fromkeys(valid_smiles))          # preserves order, deduplicates
    ...
    # --- 5. Reference-based similarities ------------------------------------
    ...
    return {
       ...
       #"valid_ratio":         round(valid_ratio,  4),
        ...
        # Distribution similarity to training set
