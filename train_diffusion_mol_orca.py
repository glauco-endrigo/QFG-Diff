...
"""ORCA-guided graph diffusion training for molecular generation."""
...
class ORCAPhotonicFragmentPrior:
    ...
    from ORCA PT-1. Photonic samples define a distribution signature,
    e o kernel usa fidelidade classica entre distribuicoes medidas:
        K(f_i, f_j) = (sum_y sqrt(p_i(y) p_j(y)))^2

    When pcss_qapi is available, this calls the ORCA backend
...
if __name__ == "__main__":
    ...
    print(f"  ORCA fragment QML: {fragment_prior.orca_status} | bank={len(fragment_prior.photonic_fragment_bank)} | calls={fragment_prior.orca_queries}")
