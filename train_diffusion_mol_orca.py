# ============================================================
# SINGLE-CELL ORCA PT-1 PHOTONIC FRAGMENT DIFFUSION NOTEBOOK
# Abra este .ipynb no Jupyter da PSNC e rode esta unica celula.
#




# Para usar o hardware real, troque para ORCA_FRAGMENT_BACKEND="ORCA-PT-1-B".
# ============================================================
import os

os.environ.setdefault("ORCA_FRAGMENT_QML", "auto")

#os.environ["ORCA_FRAGMENT_BACKEND"] = "multi_loop_simulator"
os.environ["ORCA_FRAGMENT_BACKEND"] = "ORCA-PT-1-B"
os.environ.setdefault("ORCA_FRAGMENT_BANK", "16")
os.environ.setdefault("ORCA_FRAGMENT_SAMPLES", "10")
os.environ.setdefault("ORCA_FRAGMENT_MAX_QUERIES", "64")
os.environ.setdefault("ORCA_FRAGMENT_LOGIN", "0")

# Se seus arquivos config.py / mol_metrics.py estiverem no mesmo diretorio do notebook,
# o Python ja deve encontra-los. Estes caminhos extras ajudam no Jupyter da PSNC.
import sys
for _p in [os.getcwd(), "/ORCA", "/home/jovyan/ORCA", "/home/jovyan"]:
...
=====================================================================
Integração do motor de difusão discreta com o pipeline de avaliação do Mark 2.
"""
...
LR         = 3e-4
D_MODEL    = 256
N_GEN      = 1000  # Começamos com 1000 para uma estatística robusta
GEN_BATCH  = 256

# ==============================================================================
# SUGGESTED PRODUCTION HYPERPARAMS

T_STEPS    = 1000  # Aumentar os passos de difusão dá mais "calma" ao modelo no denoising
D_MODEL    = 512   # Maior capacidade de representação
N_GEN      = 1000 # Padrão para publicação/conferência
LR         = 1e-4  # LRs menores costumam ser mais estáveis para Transformers grandes
# ==============================================================================
...
# ==============================================================================
# 3. CONVERSÃO RDKIT & SALVAMENTO (LOGICA DO MARK 2)
# ==============================================================================
...
class ORCAPhotonicFragmentPrior:
    """
    ORCA PT-1 photonic fragment-kernel prior.

    Cada fragmento BRICS vira angulos de um Time-Bin Interferometer pequeno
    do ORCA PT-1. As amostras fotonicas definem uma assinatura de distribuicao,
    e o kernel usa fidelidade classica entre distribuicoes medidas:
        K(f_i, f_j) = (sum_y sqrt(p_i(y) p_j(y)))^2

    No Jupyter/PSNC, se pcss_qapi estiver disponivel, isto chama o backend ORCA
    configurado. Fora dele, usa fallback local para manter entrada/saida intactas.
    """
    def __init__(self, adj, nodes, train_smiles, atom_labels, num_node_cls, num_edge_cls):
        ...
        self.orca_tbi = None
        self.orca_queries = 0
        self.orca_backend_name = os.getenv("ORCA_FRAGMENT_BACKEND", "multi_loop_simulator")
        self.orca_n_samples = int(os.getenv("ORCA_FRAGMENT_SAMPLES", "24"))
        self.orca_max_queries = int(os.getenv("ORCA_FRAGMENT_MAX_QUERIES", "96"))
        self.orca_status = "local-photonic-fallback"
        self._init_orca_backend()
            ...
            for frag_smi in fragments:
                ...
                for bond in frag.GetBonds():
                    ...
                    if a.GetAtomicNum() == 0 or b.GetAtomicNum() == 0:
                        continue
                    ca, cb = self.atom_to_cls.get(a.GetAtomicNum()), self.atom_to_cls.get(b.GetAtomicNum())
    ...
    def _finalize(self):
        ...
        self._build_photonic_fragment_bank()

    def _init_orca_backend(self):
        mode = os.getenv("ORCA_FRAGMENT_QML", "auto").strip().lower()
        if mode in {"0", "off", "false", "none", "local"}:
            return
        try:
            if os.getenv("ORCA_FRAGMENT_LOGIN", "0").strip().lower() in {"1", "true", "yes"}:
                from pcss_qapi import AuthorizationService
                AuthorizationService.login()
            from pcss_qapi.orca.provider import OrcaProvider
            provider = OrcaProvider()
            backend = provider.get_backend(self.orca_backend_name)
            self.orca_tbi = backend.get_tbi()
            self.orca_status = f"orca-tbi:{self.orca_backend_name}"
        except Exception as exc:
            self.orca_status = f"local-photonic-fallback:{type(exc).__name__}"
            self.orca_tbi = None

    def _build_photonic_fragment_bank(self):
        if not self.clean_fragment_counts:
            return
        # Banco pequeno: PT-1 real e valioso, entao evitamos milhares de chamadas.
    ...
    @staticmethod
    def _canonical_outcome(outcome):
        if isinstance(outcome, np.ndarray):
            outcome = outcome.tolist()
        if isinstance(outcome, (list, tuple)):
            return tuple(int(x) for x in outcome)
        return str(outcome)

    def _samples_to_signature(self, samples):
        counts = Counter()
        if isinstance(samples, dict):
            iterable = samples.items()
            for outcome, count in iterable:
                counts[self._canonical_outcome(outcome)] += float(count)
        else:
            for outcome in samples:
                counts[self._canonical_outcome(outcome)] += 1.0
    ...
    def _photonic_signature(self, mol, cache_key=None):
        ...
        if cached is not None:
            return cached
        signature = None
        if self.orca_tbi is not None and self.orca_queries < self.orca_max_queries:
            try:
                samples = self.orca_tbi.sample(
                    input_state=[1, 0, 1, 0],
                    theta_list=[float(x) for x in self._photonic_angles(mol)],
                    n_samples=self.orca_n_samples,
                    output_format="dict",
                )
                self.orca_queries += 1
                signature = self._samples_to_signature(samples)
            except TypeError:
                samples = self.orca_tbi.sample(
                    input_state=[1, 0, 1, 0],
                    theta_list=[float(x) for x in self._photonic_angles(mol)],
    ...
    def _photonic_kernel(sig_a, sig_b):
        ...
        affinity = sum(math.sqrt(sig_a.get(k, 0.0) * sig_b.get(k, 0.0)) for k in keys)
    ...
    def _project_molecule(self, mol, salt, max_atoms):
        ...
        if len(frag_mols) < 2:
            return None
        try:
            builder = BRICS.BRICSBuild(frag_mols, onlyCompleteMols=True, uniquify=True,
                                       scrambleReagents=False, maxDepth=max(1, min(3, len(frag_mols))))
        except TypeError:
...
if __name__ == "__main__":
    ...
    fragment_prior = ORCAPhotonicFragmentPrior(adj_m, nod_f, train_smiles, config.ATOM_LABELS, NUM_ATOM_TYPES, NUM_BOND_TYPES).to(device)
    print(f"  ORCA fragment QML: {fragment_prior.orca_status} | banco={len(fragment_prior.photonic_fragment_bank)} | chamadas={fragment_prior.orca_queries}")
    ...
    n_tensor, e_tensor = generate_molecules(model, fwd_proc, N_GEN, GEN_BATCH, config.NUM_ATOMS, fragment_prior=fragment_prior)
